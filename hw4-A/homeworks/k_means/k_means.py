...
def calculate_centers(
    ...
    new_centers = np.empty((num_centers, num_dims))

    for i in range(num_centers):
        inds_in_class = np.nonzero(classifications == i)[0]
        if inds_in_class.size > 0:
            new_centers[i,:] = np.mean(data[inds_in_class, :], axis=0)
...
def cluster_data(data: np.ndarray, centers: np.ndarray) -> np.ndarray:
    ...
    """
    return np.argmin(np.linalg.norm(centers[:, np.newaxis, :] - data, axis=2), axis=0)
...
def lloyd_algorithm(
    ...
    num_points, num_dims = data.shape
    trained_centers = np.random.uniform(low=np.min(data), high=np.max(data), size=(num_centers, num_dims))
    previous_centers = 1 * np.ones(((num_centers, num_dims)))
    errors = []
    stop_crit = np.max(np.abs(trained_centers - previous_centers))

    while stop_crit > epsilon:
        previous_centers = trained_centers
        classifications = cluster_data(data, previous_centers)
        trained_centers = calculate_centers(data, classifications, num_centers)
        errors.append(calculate_error(data, trained_centers))
        stop_crit = np.max(np.abs(trained_centers - previous_centers))

    return trained_centers, errors
