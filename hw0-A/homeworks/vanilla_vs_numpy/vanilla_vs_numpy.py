...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    num_elements = len(x)
    A_transpose_x = vanilla_matmul(vanilla_transpose(A), x)
    A_x = vanilla_matmul(A, x)
    B_transpose_y = vanilla_matmul(vanilla_transpose(B), y)

    gradient_f_wrt_x = []
    for n in range(num_elements):
        gradient_f_wrt_x.append(A_transpose_x[n] + A_x[n] + B_transpose_y[n])
    
    return gradient_f_wrt_x
...
def numpy_solution(
    ...
    """
    gradient_f_wrt_x = np.matmul((A + np.transpose(A)), x) \
                       + np.matmul(np.transpose(B), y)
    return gradient_f_wrt_x
